...
bool solve(bigint n) {
	bigint s = n * n * 2 + n * ((bigint)2 * r - 1);
	return s > paint;
}

bigint binary_answer(bigint l, bigint r, bool(*solve)(bigint)) {
	if (l > r) return -1; // base case; no found element

	bigint m = l + (r - l) / 2;

	if (solve(m)) { // we met some 'yes' here
		bigint answer = binary_answer(l, m - 1, solve);
		if (answer != -1) {
			return answer;
		}
		else {
...
int main() {
	...
	for (int t = 1; t <= T; t++) {
		...
		bigint answer = binary_answer(1, pow(10, 18), solve) - 1;
